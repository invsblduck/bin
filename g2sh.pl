...
# scp shiz to $host
#if ($host !~ /^sandwichmaker(\.corp)?(\.expertcity\.com)?$/) {
#
#    @files = qw( ~/g2sh.pl ~/.vimrc ~/.toprc ~/.bash_profile
#                 ~/.bashrc ~/.bash_aliases ~/.inputrc ~/.motd ~/.motd.old);
#
#    system ("/usr/bin/scp -r @files $host:");
#}
